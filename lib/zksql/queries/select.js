...
export const execute_select_query = async (query) => {
    ...
    const where = parse_where_expression(query.where, froms, fields);
...
const parse_where_expression = (expression, froms, fields) => {
    if (expression?.type === "column_ref")
        return parse_where_expression_column_ref(expression, froms, fields);

    if (expression?.left)
        expression.left = parse_where_expression(expression.left, froms, fields);
    if (expression?.right)
        expression.right = parse_where_expression(expression.right, froms, fields);
    return expression
}
...
const parse_where_expression_column_ref = (expression, froms, fields) => {
    console.log({ fields, froms_fields: froms.reduce((acc, from) => [...acc, ...from.columns], []) });
    for (const { ref, table, column } of fields) {
        if (expression?.table) {

        }
        console.log({ ref, table, column })
    }
    if (expression?.table) {
    ...
    if (!column)
        throw new Error(
            `Error: column ${expression.column} not found.`
        );
    return {
        type: "column_ref",
        column,

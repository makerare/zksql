...
export const select_query_to_table = (query, froms, fields) => {
    ...
    table.program.functions.push(...select_functions(table, froms, fields));
...
const select_functions = (table, froms, fields) => {
    return froms.map(
        (from, index) => select_process_function(table, from, fields, index,)
    );
}
...
const select_process_function = (to, from, fields, index) => {
    ...
    return {
        name: `process_${from.name}_${to.name}`,
        inputs: [
            {
                name: "r0",
                type: {

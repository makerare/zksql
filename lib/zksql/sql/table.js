...
const table_program_from_columns = (table_name, columns) => {
  const structs = description_structs_from_columns(table_name, columns);
...
const description_structs_from_columns = (columns) => {
  const struct_attributes = this.columns.map(column_to_attribute);
  const struct = {

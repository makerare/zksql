...
export class Table {
  constructor(database_name, table_name, as, crudable) {
    this.program = new Program(table_name);
    this.database = database_name;
    this.name = table_name;
    this.ref = as || table_name;
    this.crudable = crudable || false;
  }
  ...
  load_from_columns(columns) {
    ...
    return this.program.load_from_code(this.program_code);
  }
  ...
  _load_crudable() {
    this.crudable = inter(
      new Set(
        this.program.structs.map(({ name }) => name)
      ),
      new Set([
        this.insert_transition_name,
        this.update_transition_name,
        this.delete_transition_name
      ])
    ).size === 3;
  }

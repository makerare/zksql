...
const instruction_to_code = (line, block_name) => {
  return opcodes_to_code[line.opcode](line, block_name);
}


const opcodes_to_code = {
  "cast": (line, block_name) => {
    const { inputs, output, opcode } = line;
    const inputs_code = inputs.map(({ name }) => name).join(" ");
    const type_str = type_to_code(output.type, true);
    return `${opcode} ${inputs_code} into ${output.name} as ${type_str};`;
  },
}
...
const as_to_code = (
  ...
  const { identifier, specify_visibility } = optional || {};
...
const type_to_code = (type, specify_visibility) => {
  ...
  const visibility = specify_visibility && type.visibility ? `.${type.visibility}` : ``;

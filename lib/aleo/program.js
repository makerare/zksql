...
export class Program {
  ...
  get finalize_code() {
    return this.finalizes.map(finalize_to_code).join("\n");
  }
}
...
const program_blocks_to_definitions = (blocks) => {
  ...
  for (const block of blocks) {
    ...
    definitions[definition_name].push({ type: _, ...block });
  }
...
const import_to_code = (import_) => {
  return `import ${import_.data.name}.aleo;`;
};

const struct_to_code = (struct) => {
  const attributes = struct.data.fields.map(
    (field) => as_to_code(field.name, field.type)
  ).join("\n");
  return `struct ${struct.data.name}:\n${attributes}`;
}

const as_to_code = (name, type, identifier) => {
  return (
    `    `
